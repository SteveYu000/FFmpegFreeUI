...
注意：这个顺序只能保证“同一事件或同一 Ext 阶段”内部的顺序，不能改变不同生命周期节点之间的固定顺序。所以，一个使用 Ext task.after-complete，另一个使用官方 task.completed，则由程序生命周期决定先后，管理页面不能把两者对调。官方插件自行启动后台任务，只能保证“启动后台任务”的顺序，不能保证后台任务完成的顺序。只有两个插件都监听官方api或者Ext，才能排序。
